...
def calcClickFlag(pos):
    ...
    for i in range(3):
        ...
        if g_BGRECT[i].collidepoint(pos):
            # print(i, 'collidepoint', g_ClickFlag.count(1), tmpPie[i])
    ...
    # print(g_FirstClick, g_SecondClick, g_ClickFlag, 'click')
    MovePie()
...
def MovePieAnimation():
    global g_AutoFlag, g_animation, g_firstPie, g_basePie, g_ClickFlag,g_FirstClick, g_SecondClick 
    leftOk = False
    topOk = False
    left = g_basePie.left + (g_basePie.width-g_firstPie.width)//2
    top = g_basePie.top - g_firstPie.height-2
    if g_firstPie.left < left:
        g_firstPie.left += 1
    elif g_firstPie.left > left:
        g_firstPie.left -= 1
    else:
        leftOk = True
        
    if g_firstPie.top < top:
        g_firstPie.top += 1
    elif g_firstPie.top > top:
        g_firstPie.top -= 1
    else:
        topOk = True
    
    if leftOk and topOk and g_animation:
        # print(g_animation, '@@@@@@@@@@')
        tmpPie = [A_PieRect, B_PieRect, C_PieRect]
        g_animation = False

        tmpPie[g_FirstClick].remove(g_firstPie)
        tmpPie[g_SecondClick].append(g_firstPie)
        # for item in tmpPie:
        #     print(item, '******')
        if g_AutoFlag ==0:
            g_ClickFlag = [0,0,0]
